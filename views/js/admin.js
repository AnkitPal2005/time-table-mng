...
async function saveLecture() {
  ...
  try {
    const response = await fetch("/api/auth/lectures", {
      method: "POST",
...
async function renderTable() {
  ...
  try {
    const response = await fetch(
      `/api/auth/showLectures/${selectedTeacher}`
    );
    const data = await response.json();
    if (response.ok) {
...
async function handleAdjust(username, day) {
  try {
    const res = await fetch("/api/auth/getlecturesleave", {
      method: "POST",
      headers: {
...
async function handleApprove(username, day, status) {
  try {
    const res = await fetch("/api/auth/approveleave", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({ username, day }),
...
try{
    const res = await fetch("/api/auth/assignteacher", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
...
async function assignLectureToTeacher(teacher,day,period,startTime,subject,room,endTime) {
  try {
    const res = await fetch("/api/auth/assignLecture", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({
